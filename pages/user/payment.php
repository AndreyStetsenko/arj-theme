...
<script>
	...
	$('.alert').each(function() {
		var text = $(this).text(); // берем текст текущего элемента
		text = text.replace(/Выплата успешно заказана! Ожидайте обработки администратором./g, "Выплата успешно заказана. Отработка начнётся с понедельника. Срок - до 5 дней!"); // заменяем необходимое
		$(this).text(text); // выставляем новый текст в элемент
	});
